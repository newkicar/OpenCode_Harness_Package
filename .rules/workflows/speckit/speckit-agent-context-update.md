...
It then creates, replaces, or appends the managed block so that the section points at the most recent plan path when one can be discovered (`.scratch/<feature>/plan.md`).
...
When `plan_path` is omitted, the script auto-detects the most recently modified `.scratch/*/plan.md`.