...
> **PyTorch / 深度学习项目专用**。需已通过启用 L2 的 `03-pytorch-code-rule.md`（复制 `.rules/l2/03-pytorch-code-rule.md` → `.rules/`）。
> 小改动（改 lr、修 bug）仍可直接改，不需走完整实验流程。
...
2. 更新或确认需求文档中的**可量化指标**（如 val accuracy ≥ X）。
...
4. 失败则停止，不启动 multi-epoch 训练；问题记录到 `memory/progress.md`。
...
3. 更新 `memory/progress.md`：记录最佳 metric、checkpoint 路径、下次实验计划。
