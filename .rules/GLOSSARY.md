...
| **收工 (Session End)** | 完成一个独立任务单元后的清理和 Memory 更新动作 | 更新 `memory/progress.md` 记录完成项和下一步 |
...
| 术语 | 对应文件 | 说明 |
|------|---------|------|
| **L1 核心规则** | `00-core.md`, `01-ponytail.md`, `plugins/*` | 永远加载，不选择性跳过 |
| **L2 领域规则** | `02-deepagents-code-rule.md`, `03-pytorch-code-rule.md` | AI 自动检测项目类型后按需加载 |
| **L3 工作流** | `workflows/*.md`（不含 `speckit/`） | 按任务类型三选一 |
| **L3 Speckit** | `workflows/speckit/*.md` | 可选激活的子体系，引用 L4 memory/ 模板 |
...
| 术语 | 工具名 | 说明 |
|------|-------------|------|
| **全量写文件** | `write` | 覆盖整个文件内容，`memory/` 目录唯一允许的写入方式 |
| **部分修改** | `edit` | 搜索替换文件中的特定段落，**禁止用于 `memory/` 目录** |
| **读取文件** | `read` | 读取文件完整内容，`memory/` 更新前的必要步骤 |
| **PreToolUse** | TypeScript plugin | 文件写入**前**执行的安全拦截 |
| **PostToolUse** | TypeScript plugin | 文件写入**后**执行的审计扫描 |

## 规则动作

| 术语 | 含义 | 效果 |
|------|------|------|
| **BLOCK** | 阻止操作 | 系统取消该工具调用，显示错误信息 |
