...
> **Agent 项目专用**。默认存放在 `.rules/l2/`，AI 检测到 `create_deep_agent()` 时自动加载。
> 安全、泛化、正则边界见 `.rules/00-core.md`。
...
1. **语义匹配原则**：详见 00-core §4。严禁用正则对非结构化自然语言做语义匹配。
...
4. **正则使用边界**：见 00-core §4。Input Guardrails 层可用正则做强格式预处理，再交给 LLM。
...
> 通用安全红线见 `.rules/00-core.md` §2。DeepAgents 平台增量：
