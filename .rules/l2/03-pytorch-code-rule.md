...
> **深度学习项目专用**。默认存放在 `.rules/l2/`，AI 检测到 `import torch` 时自动加载。
> 进阶主题（分布式/AMP/Windows/CUDA/导出）见 `.agents/skills/pytorch-advanced/SKILL.md`，按需激活。
> 安全、泛化、架构原则见 `.rules/00-core.md`。
> 最后更新：2026-06-22
...
6. **禁止静默吞训练异常**：NaN loss、空 DataLoader、CUDA OOM 必须显式报错或中断（见 00-core §2a）。
...
> 通用 verify 见 `.rules/workflows/verify-changes.md`。
